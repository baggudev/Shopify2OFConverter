...
var OnFufillmentTranslators = {
    ...
    "bill_to_country": function(data) {
        var countryCode = data["Billing Country"];
        return countryCode.toUpperCase() === "CR" ? "CS" : countryCode;
    },
    ...
    "personalization": function(data) {
        // the notes will come with "" we need to trim.
        // check if there are leading/trailing quotes

        var notes = data["Notes"].split("\n").join(); // eliminate the newline characters
        var hasQuotes = (notes.substr(0,1) === "\"") || (notes.substr(0,1) === "\'");
        hasQuotes ? return notes.substr(1, notes.length - 2) : return notes;
    },
    "pos1": function(data) {
        return "";
    },
    ...
    "ship_to_country": function(data) {
        // required
        // Special Country code for Serbia and Montenegro. CR -> CS
        var countryCode = data["Shipping Country"];
        return countryCode.toUpperCase() === "CR" ? "CS" : countryCode;
    },
